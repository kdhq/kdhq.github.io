/*!
 * KarukajNav 1.4
 * Custom navigation system for Karukaj
 * Based on Meny by Hakim El Hattab
 * MIT licensed
 */
...
var KarukajNav = {
	...
	// Creates a new instance of KarukajNav
	create: function( options ) {
		return (function(){
			...
			function setupWrapper() {
				...
				// position
				KarukajNav.addClass( dom.wrapper, 'karukaj-nav-' + config.position );

				originalStyles.wrapper = dom.wrapper.style.cssText;

				dom.wrapper.style[ KarukajNav.prefix( 'perspective' ) ] = '800px';
				dom.wrapper.style[ KarukajNav.prefix( 'perspectiveOrigin' ) ] = contentsTransformOrigin;
			}
			...
			function setupCover() {
				...
				if( supports3DTransforms ) {
					dom.cover.style[ KarukajNav.prefix( 'transition' ) ] = 'all ' + config.transitionDuration +' '+ config.transitionEasing;
				}
			...
			/**
			 * The menu element that folds out upon activation.
			 */
			function setupMenu() {
				// Shorthand
				...
				if( supports3DTransforms ) {
					style[ KarukajNav.prefix( 'transform' ) ] = menuTransformClosed;
					style[ KarukajNav.prefix( 'transformOrigin' ) ] = menuTransformOrigin;
					style[ KarukajNav.prefix( 'transition' ) ] = 'all ' + config.transitionDuration +' '+ config.transitionEasing;
				}
				else {
					KarukajNav.extend( style, menuStyleClosed );
				}
			}
			...
			function setupContents() {
				...
				if( supports3DTransforms ) {
					style[ KarukajNav.prefix( 'transform' ) ] = contentsTransformClosed;
					style[ KarukajNav.prefix( 'transformOrigin' ) ] = contentsTransformOrigin;
					style[ KarukajNav.prefix( 'transition' ) ] = 'all ' + config.transitionDuration +' '+ config.transitionEasing;
				}
				else {
					style.position = style.position.match( /relative|absolute|fixed/gi ) ? style.position : 'relative';
					KarukajNav.extend( style, contentsStyleClosed );
				}
			}
			...
			function bindEvents() {
				...
				if( 'ontouchstart' in window ) {
					if( config.touch ) {
						KarukajNav.bindEvent( document, 'touchstart', onTouchStart );
						KarukajNav.bindEvent( document, 'touchend', onTouchEnd );
					}
					else {
						KarukajNav.unbindEvent( document, 'touchstart', onTouchStart );
						KarukajNav.unbindEvent( document, 'touchend', onTouchEnd );
					}
				}

				if( config.mouse ) {
					KarukajNav.bindEvent( document, 'mousedown', onMouseDown );
					KarukajNav.bindEvent( document, 'mouseup', onMouseUp );
					KarukajNav.bindEvent( document, 'mousemove', onMouseMove );
				}
				else {
					KarukajNav.unbindEvent( document, 'mousedown', onMouseDown );
					KarukajNav.unbindEvent( document, 'mouseup', onMouseUp );
					KarukajNav.unbindEvent( document, 'mousemove', onMouseMove );
				}
			}
			...
			function open() {
				if( !isOpen ) {
					...
					KarukajNav.addClass( dom.wrapper, 'karukaj-nav-active' );
					...
					if( supports3DTransforms ) {
						...
						KarukajNav.bindEventOnce( dom.wrapper, 'transitionend', function() {
							KarukajNav.dispatchEvent( dom.menu, 'opened' );
						} );

						dom.cover.style.opacity = 1;

						dom.contents.style[ KarukajNav.prefix( 'transform' ) ] = contentsTransformOpened;
						dom.menu.style[ KarukajNav.prefix( 'transform' ) ] = menuTransformOpened;
					}
					// ...fall back on JS animation
					else {
						menuAnimation && menuAnimation.stop();
						menuAnimation = KarukajNav.animate( dom.menu, menuStyleOpened, 500 );
						contentsAnimation && contentsAnimation.stop();
						contentsAnimation = KarukajNav.animate( dom.contents, contentsStyleOpened, 500 );
						coverAnimation && coverAnimation.stop();
						coverAnimation = KarukajNav.animate( dom.cover, { opacity: 1 }, 500 );
					}

					KarukajNav.dispatchEvent( dom.menu, 'open' );
				}
			}
			...
			function close() {
				if( isOpen ) {
					...
					KarukajNav.removeClass( dom.wrapper, 'karukaj-nav-active' );

					// Use transforms and transitions if available...
					if( supports3DTransforms ) {
						// 'webkitAnimationEnd oanimationend msAnimationEnd animationend transitionend'
						KarukajNav.bindEventOnce( dom.wrapper, 'transitionend', function() {
							KarukajNav.dispatchEvent( dom.menu, 'closed' );
						} );

						dom.cover.style.visibility = 'hidden';
						dom.cover.style.opacity = 0;

						dom.contents.style[ KarukajNav.prefix( 'transform' ) ] = contentsTransformClosed;
						dom.menu.style[ KarukajNav.prefix( 'transform' ) ] = menuTransformClosed;
					}
					// ...fall back on JS animation
					else {
						menuAnimation && menuAnimation.stop();
						menuAnimation = KarukajNav.animate( dom.menu, menuStyleClosed, 500 );
						contentsAnimation && contentsAnimation.stop();
						contentsAnimation = KarukajNav.animate( dom.contents, contentsStyleClosed, 500 );
						coverAnimation && coverAnimation.stop();
						coverAnimation = KarukajNav.animate( dom.cover, { opacity: 0 }, 500, function() {
							dom.cover.style.visibility = 'hidden';
							KarukajNav.dispatchEvent( dom.menu, 'closed' );
						} );
					}
					KarukajNav.dispatchEvent( dom.menu, 'close' );
				}
			}

			/**
			 * Unbinds KarukajNav and resets the DOM to the state it
			 * was at before KarukajNav was initialized.
			 */
			function destroy() {
			...
			function onTouchStart( event ) {
				...
				KarukajNav.bindEvent( document, 'touchmove', onTouchMove );
			}
			...
			function onTouchEnd( event ) {
				KarukajNav.unbindEvent( document, 'touchmove', onTouchMove );
			...
			return {
				...
				addEventListener: function( type, listener ) {
					...
					dom.menu && KarukajNav.bindEvent( dom.menu, type, listener );
				},
				removeEventListener: function( type, listener ) {
					dom.menu && KarukajNav.unbindEvent( dom.menu, type, listener );
				}
			};
	...
	dispatchEvent: function( element, type, properties ) {
		if( element ) {
			...
			KarukajNav.extend( event, properties );
